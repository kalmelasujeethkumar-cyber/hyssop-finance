...
| `ISSUE-017` | RESOLVED | `PHASE-02-DATABASE` | The Phase 02 migration-drift check passed `--shadow-database-url` pointing at the live development database. `prisma migrate diff` **resets the shadow database before use**, so the check silently destroyed `hyssop_finance_dev`: its `_prisma_migrations` history, its least-privilege grants, and all 48 seeded transactions. The drift result was correct; the procedure was destructive. | Found at Phase 03 preflight: `prisma migrate status` reported both migrations unapplied while the tables still existed, and every table had `0` rows. No npm script or document had ever referenced `--shadow`, so the repository was not affected, only the local database | Replaced with `scripts/migration-drift.mjs` (`npm run db:drift`): it creates a disposable `hyssop_finance_shadow` database owned by the migration role, guards the name to end with `_shadow`, refuses the development database explicitly, always drops the shadow database in a `finally` block, and additionally compares the live development database with the read-only `--from-url` check. The development database was restored through the documented `reset` -> `db:migrate` -> `db:seed` workflow | None. The guard was verified to refuse `hyssop_finance_dev`, and the development database still holds 48 transactions and 2 recorded migrations after a drift run |

| `ISSUE-018` | RESOLVED | `PHASE-02-DATABASE` | The concurrent reference-allocation database test failed intermittently with `PrismaClientKnownRequestError: Transaction API error: Unable to start a transaction in the given time` while firing 12 concurrent interactive transactions. | Reproduced on the first `npm run test:db` run at Phase 03 preflight; `node_modules/@prisma/client/runtime/library.d.ts` documents the defaults `maxWait ?= 2000` and `timeout ?= 5000`, and the failing test allocates 12 references through the `id_sequence` upsert whose row lock serializes them by design. The failure is a client-side acquisition wait, not a duplicate reference and not a lock failure | Raised the shared client's transaction waits explicitly to `maxWait` 15 s and `timeout` 30 s (`DEC-063`) instead of reducing the test's concurrency or weakening its assertion | `npm run test:db` passed 48 of 48 tests on two consecutive runs, and the full `npm run verify` gate (lint, typecheck, script typecheck, 103 API tests, 19 web tests, build, format) passed afterwards |
